...
class DuckChessEnv(gym.Env):
	...
	def _get_black_action(self):
		...
		if len(valid_actions) > 0:
			if self.engine.phase == 'move_piece':
				# --- GREEDY LOGIC ---
				# Check all valid moves. If a move results in a capture, collect it.
				capture_actions = []
				...
				# If there are any captures available, randomly choose one of them.
				if capture_actions:
					return np.random.choice(capture_actions)
			
			# If no captures (or if it's the duck placement phase), play randomly
	...
	def _save_replay(self, reason):
		...
		try:
			...
			# Print a subtle message for periodic saves, and a loud one for crashes
	...
	def step(self, action):
		...
		try:
			...
			# --- PERIODIC SAVING ---
			# Every 50 episodes, save the game so the user can review it
