...
Each iteration:
  1. GENERATE  MCTS games with the current model (gen_mcts_data): a self-play
     fraction (general improvement) + a fraction vs Peter d3 (targeted pressure
     on the actual wall).
  2. TRAIN     both heads on a replay window of recent iterations (train_exit).
  3. EVALUATE  the new model WITH MCTS vs Peter d2 and d3 (eval_search).
  4. ADVANCE   current ← new model (AlphaZero-style always-accept), while keeping
     a guarded copy of the best-by-(d3,d2) checkpoint so a late regression can
     never lose the best model.

This is the only identified path at the depth-3 wall: raw PPO and one-shot search
both plateau; iterating search→network→search is what compounds.

Usage (full build, after antiexploit_v2 finishes):
  python -m DuckChess_Game.SBThree.run_exit \
      --base models/duck_ppo/antiexploit_v2/ax_final.zip \
      --out-dir models/duck_ppo/exit --iters 8 --games 600 --sims 200 \
      --workers 8 --peter-frac 0.30 --eval-games 16 --eval-sims 200

Smoke (proves the loop end-to-end; the model won't be good):
  python -m DuckChess_Game.SBThree.run_exit --base models/duck_ppo/v2/v2_final.zip \
      --out-dir models/duck_ppo/exit_smoke --iters 1 --games 4 --sims 20 \
      --workers 2 --peter-frac 0.5 --eval-games 2 --eval-sims 20 --epochs 2
"""
...
def run(base, out_dir, *, iters=8, games=600, sims=200, workers=8,
        peter_frac=0.30, peter_depth=3, window=4, epochs=10, lr=1e-4,
    ...
    best_key = (-1.0, -1.0)   # (d3_score, d2_score): break ties on d3

    print(f"=== ExIt | base={os.path.basename(base)} | {iters} iters | "
          f"{games} games/iter (peter_frac={peter_frac}) | sims={sims} | "
          f"window={window} files ===", flush=True)
    ...
    for it in range(1, iters + 1):
        ...
        if peter_games > 0:
            files_this.append(generate(
                ...
                opponent="peter", peter_depths=(peter_depth,), seed=seed + it * 1000 + 7))
        gen_s = time.time() - t0
        ...
        print(f"[run_exit] iter {it} DONE in {(time.time() - t_it) / 60:.1f} min | "
              f"d2 {r2['wins']}/{r2['losses']}/{r2['draws']} (score {r2['score']:.2f}) | "
              f"d3 {r3['wins']}/{r3['losses']}/{r3['draws']} (score {r3['score']:.2f}) | "
              f"{'NEW BEST' if is_best else 'kept prior best'}", flush=True)

    print(f"\n=== ExIt finished. Best (d3,d2)={best_key} -> {best_path} ===", flush=True)
    return best_path
...
def main():
    ...
    p.add_argument("--peter-frac", type=float, default=0.30,
                   help="fraction of generated games played vs Peter d3")
    p.add_argument("--peter-depth", type=int, default=3)
    ...
    run(args.base, args.out_dir, iters=args.iters, games=args.games, sims=args.sims,
        workers=args.workers, peter_frac=args.peter_frac, peter_depth=args.peter_depth,
