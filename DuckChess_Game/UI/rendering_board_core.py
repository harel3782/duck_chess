...
class BoardCoreRenderingMixin:
	"""Handles the physical 8x8 grid, highlights, and piece rendering."""

	def _draw_base_board(self):
		"""Draws the checkerboard squares and coordinate notation [cite: 138-142]."""
		...
		for r in range(8):
			for c in range(8):
				...
				txt_col = WHITE_COLOR if (r + c) % 2 != 0 else BLACK_SQ_COLOR
	...
	def draw_duck(self, r, c):
		"""Renders the duck sprite centered on a square [cite: 142-143]."""
		x, y = self.get_screen_pos(r, c)
	...
	def draw_game(self, hidden_square=None):
		"""The master render function for active gameplay frames [cite: 155-169]."""
		...
		bw = self.sq_size * 8
		pygame.draw.rect(self.screen, (20, 20, 20), (self.board_x - 2, self.board_y - 2, bw + 4, bw + 4), width=2)
			...
			for c in range(8):
				...
				if is_live and not getattr(self, 'promotion_pending', False):
					...
					if self.phase == 'move_piece':
						if getattr(self, 'selected_square', None) == (r, c): pygame.draw.rect(self.screen, HIGHLIGHT, (x, y, self.sq_size, self.sq_size))
				...
				p = b[r][c]
				if p:
					if p.type == 'K' and hasattr(self, 'is_in_check') and self.is_in_check(p.color, b): self._draw_highlight_square(r, c, (235, 60, 60, 180))
		...
		if getattr(self, 'dragging', False) and self.drag_piece and is_live:
			...
			if k in self.scaled_images: self.screen.blit(self.scaled_images[k], (mx - self.drag_offset[0], my - self.drag_offset[1]))
