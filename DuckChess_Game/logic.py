...
class GameLogicMixin:
    ...
    def init_board(self):
        setup = [(ROOK, 0, 0), (KNIGHT, 0, 1), (BISHOP, 0, 2), (QUEEN, 0, 3), (KING, 0, 4), (BISHOP, 0, 5),
                 ...
                 (KNIGHT, 7, 6), (ROOK, 7, 7)]
        for t, r, c in setup:
            self.board[r][c] = Piece('b' if r == 0 else 'w', t)
        for c in range(8):
            self.board[1][c], self.board[6][c] = Piece('b', PAWN), Piece('w', PAWN)
    ...
    @staticmethod
    def calculate_material_score(board_state):
        score = 0
        for r in range(8):
            for c in range(8):
                p = board_state[r][c]
                if p:
                    score += PIECE_VALUES[p.type] * (1 if p.color == 'w' else -1)
        return score
    ...
    def get_piece_legal_moves(self, r, c):
        ...
        if p.type == KING:
            ...
            for dr, dc in dirs:
                ...
                if ok(nr, nc) and (nr, nc) != self.duck_pos:
                    t = self.board[nr][nc]
                    if not t or t.color != p.color:
                        moves.append((nr, nc))
            if not p.has_moved:
                if self.can_castle(r, True):
                    moves.append((r, 6))
                if self.can_castle(r, False):
                    moves.append((r, 2))
            return moves
        ...
        if dirs:
            for dr, dc in dirs:
                for i in range(1, 8):
                    nr, nc = r + dr * i, c + dc * i
                    if not ok(nr, nc) or (nr, nc) == self.duck_pos:
                        break
                    t = self.board[nr][nc]
                    if not t:
                        moves.append((nr, nc))
    ...
    def can_castle(self, r, is_ks):
        ...
        for cl in path_cols:
            if self.board[r][cl] or (r, cl) == self.duck_pos:
                return False
        return True
            ...
            for i in range(1, 8):
                ...
                if p:
                    if p.color == enemy:
                        if p.type == QUEEN:
                            return True
                        if p.type == ROOK and (dr == 0 or dc == 0):
                            return True
                        if p.type == BISHOP and (dr != 0 and dc != 0):
                            return True
                    break
    ...
    def execute_move(self, start, end, animated=True):
        ...
        if p.type == KING and abs(sc - ec) == 2:
            ...
            if is_capture:
                if p.type == PAWN:
                    move_str += self.get_notation_coords(sr, sc)[0]
                move_str += "x"
                sound = 'capture'
            move_str += self.get_notation_coords(er, ec)

        # 4. Execute Board Updates
        ...
        next_ep = None
        if p.type == PAWN and abs(sr - er) == 2:
            next_ep = ((sr + er) // 2, sc)
    ...
    def promote_pawn(self, type_char):
        ...
        enemy_color = 'b' if self.turn == 'w' else 'w'
        if self.is_in_check(enemy_color):
            if "+" not in self.current_move_str:
    ...
    def place_duck(self, pos, animated=True):
        if self.board[pos[0]][pos[1]] or pos == self.prev_duck_pos:
            return
    ...
    def ai_turn(self):
        ...
        if not self.waiting_for_ai:
            return
        if pygame.time.get_ticks() - self.ai_wait_start < 400:
            return
